...
export default function ContactSection() {
  ...
  // Side facts pulled from messages so each label/value translates as a pair.
  const facts: Array<[string, string]> = [
    [t("facts.responseTimeLabel"), t("facts.responseTimeValue")],
    [t("facts.availabilityLabel"), t("facts.availabilityValue")],
    [t("facts.basedLabel"),        t("facts.basedValue")],
    [t("facts.areasLabel"),        t("facts.areasValue")],
  ]
          ...
          {state.status === "success" ? (
            <div className="rounded-md border border-success/25 bg-success-bg px-6 py-6">
              ...
              <div className="mt-1 text-sm leading-[1.55] text-success-fg">
                {/* The server action returns its own success message; if it's
                    blank (e.g. action stub returns no message) fall back to
                    the localized default. */}
                {state.message || t("success.messageDefault")}
              </div>
            </div>
          ) : (
            <form action={formAction} className="flex flex-col">
              ...
              <Field id="message" label={t("form.messageLabel")} required>
                <textarea
                  id="message"
                  name="message"
                  rows={5}
