-- =============================================================================
-- Migration: add sub_jurisdiction support for IPSAS Advisor
...
-- 1. Add subgroup_id column to profiles (references org_subgroups)
--    Nullable: not every user needs to be in a subgroup.
ALTER TABLE profiles
  ADD COLUMN subgroup_id uuid REFERENCES org_subgroups(id) ON DELETE SET NULL;


-- 2. Add sub_jurisdiction column to org_subgroups
--    Nullable: not every subgroup needs its own content layer.
ALTER TABLE org_subgroups
  ADD COLUMN sub_jurisdiction text;


-- 3. Add sub_jurisdiction column to modules
...
-- 4. Add helper function: my_subgroup_id()
...
-- 5. Replace the modules RLS SELECT policy to include sub_jurisdiction check.
--    Logic:
...
DROP POLICY IF EXISTS "Approved users can view matching modules" ON modules;
...
-- 6. Replace the questions RLS SELECT policy to mirror the modules policy.
--    Questions inherit the same visibility rules as their parent module.
DROP POLICY IF EXISTS "Approved users can view questions for their modules" ON questions;
...
-- 7. Replace the modules index to include sub_jurisdiction for query performance.
