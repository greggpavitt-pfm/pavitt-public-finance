      ...
      <div className="mx-auto max-w-[1240px]">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
          ...
          <span className="font-mono text-xs tabular-nums text-ink-500">
            {String(donors.length).padStart(2, "0")} {t("countSuffix")}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-ink-200 bg-ink-200 sm:grid-cols-3 lg:grid-cols-6">
          {donors.map((d) => {
            // Donor names live in messages so e.g. "European Union" → "Union européenne".
            const name = t(`names.${d.id}` as `names.${typeof d.id}`)
            return (
              <div
                key={d.id}
                title={name}
